...
class SitioPublicoTests(unittest.TestCase):
    ...
    def test_maxmin_is_selectable_compact_and_multi_filter(self):
        for token in ('data-multi-filter=', 'data-maxmin-select=', 'step="0.1"', 'PDF etiquetas', 'primary=i.sapName', "'outputView','Ver en'", 'sleeveLine=sleeve'):
            self.assertIn(token, self.ui)
        export = (ROOT / "assets" / "export.mjs").read_text()
        self.assertIn("report.cards.length>6", export)
        self.assertIn("PEDIDOS / SEMANA", export)
