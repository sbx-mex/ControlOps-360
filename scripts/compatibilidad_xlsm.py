...
def inspect_xlsm(path: Path) -> Result:
    ...
    try:
        with zipfile.ZipFile(path) as zf:
            ...
            for sheet_name, sheet_path in sheets:
                ...
                for target in relationships(zf, rel_path).values():
                    ...
                    roles = [
                        ...
                        if (kind in {"venta", "uso"} and is_ac_source(sheet_name))
                        or kind in {"productos", "tienda", "presentaciones", "compostable"}
                    ]
            ...
            useful = any(role in {"venta", "uso", "compostable"} for source in selected for role in source.roles)
            if selected and useful:
                kinds = sorted({role for source in selected for role in source.roles})
                return Result(path.name, True, True, selected, kinds)
