...
import {addReferenceRows,createDataset,mergeDataset,operationalStores,getFilterOptions,normalize,DAY_LABELS,shortDate,shortPeriod,dateExtent,selectRecentWorkbooks,workbookFreshness,workbookKind,validateCeCo} from './engine.mjs';
...
const state={dataset:null,module:'menu',filters:{},subtabs:{normal:'Tamaños',top:'Bebidas',audit:'Negativas'},files:[],workbooks:[],sequence:0,loading:false,page:0,settings:saved(),result:null,report:null,maxminSelected:new Set(),openMulti:''};
...
function maxminView(){
 const f=filterState();f.nameMode||='sap';const r=inventory(state.dataset,{...filters(),window:'all'},state.settings.minimum);state.result=r;
 ...
 const selection=`<section class="selection-bar"><div><strong>${selected} seleccionado${selected===1?'':'s'}</strong><span>${selected?'Excel valida usos; PDF imprime etiquetas carta 3 × 4.':'Elige los productos que deseas validar o imprimir.'}</span></div><div><button data-action="select-visible">Elegir visibles</button><button data-action="clear-selection" ${selected?'':'disabled'}>Vaciar selección</button></div></section>`;
 const metrics=cards([metric('Productos con uso',n(r.items.length)),metric('Periodo',period(r),`${r.days} días observados`),metric('Uso diario','1 decimal','Conversión aplicada antes del promedio'),metric('Máximo',`Mínimo × ${r.orders===2?5:r.orders===3?4:r.orders===4?3:2}`)]);
 const grid=r.items.slice(state.page*24,state.page*24+24).map(i=>{const modeItem=state.settings.modes[i.key]||mode,c=minmaxValues(i,modeItem),primary=f.nameMode==='micros'?i.microsName:i.sapName,secondary=f.nameMode==='micros'?i.sapName:i.microsName,checked=state.maxminSelected.has(i.key),unitDetail=normalize(c.unit)!==normalize(i.unit)?` · Etiqueta ${c.unit}`:'';return `<article class="product-card${checked?' selected':''}"><div class="product-heading"><span>${esc(i.family)}</span><label class="card-select"><input type="checkbox" data-maxmin-select="${esc(i.key)}"${checked?' checked':''}><span>Imprimir</span></label></div><h3>${esc(primary)}</h3>${secondary&&secondary!==primary?`<p class="secondary-name">${esc(secondary)}</p>`:''}<p class="sku">${esc(i.woe?`Código SAP ${i.woe.sap||'—'} · DIA ${i.woe.dia||'—'}`:`Artículo ${i.id}`)}</p><div class="minmax-values"><div><span>MIN</span><strong>${n(c.minimum)}</strong></div><div><span>MAX</span><strong>${n(c.maximum)}</strong></div></div><div class="card-inputs">${field('Uso diario',dailyInput(i.minimum,`data-minimum="${esc(i.key)}" aria-label="Uso diario ${esc(primary)}"`))}${field('Etiqueta',`<select data-presentation="${esc(i.key)}" aria-label="Formato de etiqueta ${esc(primary)}">${option('unit','Unidad',modeItem)}${option('pack','Pick Pack',modeItem)}</select>`)}</div><p class="unit-line"><b>${n(i.average)}</b> ${esc(i.unit)} por día${esc(unitDetail)}</p><div class="card-foot">${i.adjusted?`<span>Ajustado manualmente</span><button data-restore="${esc(i.key)}">Restaurar</button>`:'<span>Calculado con días observados</span>'}</div>${i.p.multiplier>1?`<small class="conversion">Conversión: 1 ${esc(i.p.text)} = ${i.p.multiplier} piezas antes del promedio diario.</small>`:''}${c.minimum===null?'<p class="notice warning">Pick Pack sin equivalencia. Usa Unidad o valida el catálogo.</p>':''}</article>`;}).join('');
 return controls+selection+metrics+`<div class="product-grid">${grid||'<p class="empty-cell">Sin productos con uso positivo para estos filtros.</p>'}</div>`+pager(r.items.length);
}
function trendView(){
...
function draw(){
 ...
 $('excelButton').textContent=state.module==='maxmin'?'Excel validar ↓':'Excel ↓';$('pdfButton').textContent=state.module==='maxmin'?'PDF etiquetas ↓':'PDF ↓';
 ...
 if(r){const f=filters(),weekLabel=f.weeks?.length?`${f.weeks.length} semanas`:f.week?`Semana desde ${shortDate(f.week)}`:'',familyLabel=f.families?.length?`${f.families.length} familias`:f.family||'';state.report=reportFor(state.module,r,{store:storeLabel(),subtab:state.subtabs[state.module],modes:state.settings.modes,mode:f.display||'unit',nameMode:f.nameMode||'sap',selectedKeys:[...state.maxminSelected],orders:state.orderResults,filterLabel:[weekLabel,f.weekday!==undefined&&f.weekday!==''?DAY_LABELS[Number(f.weekday)]:'',familyLabel,f.mode||'',f.query||''].filter(Boolean).join(' · ')});}
}
