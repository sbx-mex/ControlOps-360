...
export const MODULES=[
 ...
 {id:'peak',name:'Peak Hour',caption:'Objetivos y Tarea de Ciclo',icon:'◷',type:'sales'},
...
export function cycleTasks(minutes){return CYCLE_TASKS[String(minutes)]||[];}
export function planningWeekDates(reference,today=''){
 const parsed=[reference,today].map(dateParts).filter(Boolean).sort((a,b)=>b.dayMs-a.dayMs)[0];
 if(!parsed)return[];
 const nextMonday=parsed.dayMs+(parsed.weekday===0?7:7-parsed.weekday)*DAY_MS;
 return DAY_LABELS.map((day,weekday)=>{const date=new Date(nextMonday+weekday*DAY_MS).toISOString().slice(0,10);return{day,weekday,date};});
}
export function peakPlanningRows(result,{today='',basis='average',weekdays=[]}={}){
 const selected=new Set((weekdays||[]).map(Number)),source=basis==='latest'?result.latestWeekday:result.weekday,dates=planningWeekDates(result.to,today);
 return source.filter((_,weekday)=>!selected.size||selected.has(weekday)).map((point,weekdayIndex)=>{const weekday=DAY_LABELS.indexOf(point.day),planning=dates[weekday]||{};return{...point,weekday,date:planning.date||'',basis};});
}
export function cyclePlan(result,weekday=0){
 const day=Number(weekday),label=DAY_LABELS[day]||DAY_LABELS[0];
 return result.slots.map(slot=>{const tx=slot.weekday[day],cycleMinutes=cycleFrequency(tx),tasks=cycleTasks(cycleMinutes);return{slot:slot.slot,label:`${clock(slot.slot)}–${clock(slot.slot+1)}`,period:slot.slot<28?'AM':'PM',day:label,tx,cycleMinutes,suggestedTask:tasks.length?tasks[(slot.slot+day*3)%tasks.length]:''};}).filter(row=>row.tx>0);
}
...
export function peakHour(d,f={}){
 ...
 const days=[...dates.keys()].sort(),total=transactions.size;
 ...
 function peakFor(ds,start,end){if(!ds.length)return null;let best=null;for(let i=start;i<=end-4;i++){const count=sum(ds,day=>sum(dates.get(day).slice(i,i+4),x=>x));if(count>0&&(!best||count>best.total))best={slot:i,label:`${clock(i)} - ${clock(i+4)}`,total:count,average:count/ds.length,days:ds.length};}return best?{...best,target:epsCeil(best.average+5),push:5}:null;}
 const dateSlots=days.map(date=>{const weekday=dateParts(date).weekday;return{date,weekday,day:DAY_LABELS[weekday],slots:[...dates.get(date)]};});
 const daily=days.map(date=>{const weekday=dateParts(date).weekday;return{date,weekday,day:DAY_LABELS[weekday],am:peakFor([date],0,28),pm:peakFor([date],28,48)};}),weekday=DAY_LABELS.map((day,index)=>{const comparable=days.filter(date=>dateParts(date).weekday===index);return{day,days:comparable.length,sourceDate:comparable.at(-1)||'',am:peakFor(comparable,0,28),pm:peakFor(comparable,28,48)};}),latestWeekday=DAY_LABELS.map((day,index)=>{const latest=daily.filter(item=>item.weekday===index).at(-1);return{day,days:latest?1:0,sourceDate:latest?.date||'',am:latest?.am||null,pm:latest?.pm||null};});
 return {facts,orders:total,sales:sum(facts,'total'),days:days.length,slots,activeSlots,cycleSummary,am:peakFor(days,0,28),pm:peakFor(days,28,48),daily,dateSlots,weekday,latestWeekday,...dateExtent(facts)};
}

function peakWindowValues(values,start,end){
 let best=null;for(let slot=start;slot<=end-4;slot++){const total=sum(values.slice(slot,slot+4),value=>value);if(total>0&&(!best||total>best.total))best={slot,label:`${clock(slot)} - ${clock(slot+4)}`,total};}return best;
}
export function peakTrend(result,weekday=0,{limit=8}={}){
 const day=Number(weekday),allColumns=(result.dateSlots||[]).filter(column=>column.weekday===day).sort((a,b)=>a.date.localeCompare(b.date)),columns=allColumns.slice(-Math.max(2,limit)).map((column,index,visible)=>{const total=sum(column.slots,value=>value),previous=visible[index-1],previousTotal=previous?sum(previous.slots,value=>value):null,delta=previous?total-previousTotal:null,gapDays=previous?(dateParts(column.date).dayMs-dateParts(previous.date).dayMs)/DAY_MS:null;return{...column,total,delta,deltaPct:previousTotal?delta/previousTotal:null,gapDays,am:peakWindowValues(column.slots,0,28),pm:peakWindowValues(column.slots,28,48)};}),rows=Array.from({length:48},(_,slot)=>{const values=columns.map(column=>column.slots[slot]||0),last=values.at(-1)??null,previous=values.length>1?values.at(-2):null,delta=previous===null?null:last-previous;return{slot,label:`${clock(slot)}–${clock(slot+1)}`,period:slot<28?'AM':'PM',values,last,previous,delta};}).filter(row=>row.values.some(value=>value>0));
 const latest=columns.at(-1)||null,previous=columns.at(-2)||null,changes=rows.filter(row=>row.delta!==null),biggestDrop=changes.filter(row=>row.delta<0).sort((a,b)=>a.delta-b.delta)[0]||null,biggestGain=changes.filter(row=>row.delta>0).sort((a,b)=>b.delta-a.delta)[0]||null;
 return {day:DAY_LABELS[day]||DAY_LABELS[0],weekday:day,columns,rows,latest,previous,biggestDrop,biggestGain,improving:changes.filter(row=>row.delta>0).length,declining:changes.filter(row=>row.delta<0).length,stable:changes.filter(row=>row.delta===0).length};
}
...
export function auditStore(d,f={}){
 ...
 for(const r of voids){const row=ensure(r),employee=d.employeeCatalog.get(`${r.store}|${r.employee}`),partner=r.partner||employee?.name||'',position=r.position||employee?.position||'';row.legacy||=r.legacy;row.ms=Math.min(row.ms,r.ms);row.minuteOfDay=Math.min(row.minuteOfDay,r.minuteOfDay);if(r.legacy)row.voidAmount=Math.max(row.voidAmount,Math.abs(Math.min(0,r.total)));else row.voidAmount+=Math.abs(Math.min(0,r.total));row.reasons.add(r.reason);if(r.employee)row.employees.add(r.employee);if(partner){const person=row.people.get(partner)||{name:partner,position:position||'Puesto no disponible',employee:r.employee||''};if(!person.position&&position)person.position=position;row.people.set(partner,person);}if(!r.manager||['na','0'].includes(normalize(r.manager)))row.unapproved=true;
  row.lines.push({key:r.key,at:r.ms,secTrans:r.secTrans,secDtl:r.secDtl,product:r.product,name:r.productName||d.productCatalog.get(r.product)?.name||`Producto ${r.product||'sin nombre'}`,quantity:r.quantity,total:r.total,reason:r.reason,employee:partner||'Partner no identificado',legacy:r.legacy});
  if(r.legacy&&r.payment&&!/^sin forma$/i.test(r.payment))row.payments.push({name:r.payment,amount:null});
 }
 for(const r of payments){const row=byTicket.get(r.ticketKey);if(row)row.payments.push({name:r.payment,amount:r.amount});}
 const prepared=[...byTicket.values()].map(row=>{
  const ticket=ticketByKey.get(row.key),ticketTotal=ticket?.total??null,paymentTotal=row.payments.some(p=>p.amount!=null)?sum(row.payments,p=>p.amount??0):null;
  const reasons=[...row.reasons].filter(Boolean),reasonTypes=new Set(reasons.map(auditReasonType)),people=[...row.people.values()],focus=reasonTypes.has('Foco'),reopen=reasonTypes.has('Reopen'),negativeLines=row.lines.filter(line=>line.total<0);
  const exclusive=ticketTotal!=null&&ticketTotal<0,positive=ticketTotal!=null?ticketTotal>0:paymentTotal!=null&&paymentTotal>0;
  const deletion=positive&&negativeLines.length>0,hasNegative=exclusive,risk=exclusive?'Void exclusivo':deletion?'Riesgo de borrado':reopen?'Ticket reabierto':'Sin cierre verificado';
  const evidence=ticketTotal!=null?'Total de ticket · Motor 1':paymentTotal!=null?'Neto de pagos · Motor 1 (inferencia)':'Sin total de ticket ni pagos';
  const amount=exclusive?Math.abs(ticketTotal):row.voidAmount;
  const products=row.lines.map(line=>({name:line.name,quantity:line.quantity,amount:line.total}));
  return {...row,reasons,people,partner:people.map(x=>x.name).join(', ')||'Partner no identificado',position:[...new Set(people.map(x=>x.position).filter(Boolean))].join(', ')||'Puesto no disponible',products,lines:row.lines.sort((a,b)=>a.at-b.at||Number(a.secDtl)-Number(b.secDtl)),focus,reopen,exclusive,deletion,hasNegative,negative:exclusive?ticketTotal:null,negativeLines:negativeLines.length,ticketTotal,paymentTotal,evidence,risk,amount,review:exclusive?'Cheque negativo liberado':deletion?(ticketTotal!=null?'Línea negativa en ticket positivo':'Línea negativa con pago neto positivo; confirmar total del ticket'):reopen?'Motivo inicia con r; validar reapertura':'Falta total del cheque; revisar fuente'};
 });
 const reasonsSelected=new Set((f.reasons||[]).map(String)),partnersSelected=new Set((f.partners||[]).map(String));
 const rows=prepared.filter(row=>(!reasonsSelected.size||row.reasons.some(reason=>reasonsSelected.has(reason)))&&(!partnersSelected.size||row.people.some(person=>partnersSelected.has(person.name)))).sort((a,b)=>Number(b.exclusive)-Number(a.exclusive)||Number(b.deletion)-Number(a.deletion)||Number(b.reopen)-Number(a.reopen)||b.amount-a.amount||b.ms-a.ms);
 ...
 return {items:rows,voidCount:rows.length,exclusiveCount:rows.filter(row=>row.exclusive).length,deletionCount:rows.filter(row=>row.deletion).length,inferredDeletionCount:rows.filter(row=>row.deletion&&row.ticketTotal==null).length,unverifiedCount:rows.filter(row=>!row.exclusive&&!row.deletion).length,focusCount:rows.filter(row=>row.focus).length,reopenCount:rows.filter(row=>row.reopen).length,negativeCount:rows.filter(row=>row.exclusive).length,negativeAmount:sum(rows.filter(row=>row.exclusive),row=>Math.abs(row.negative)),totalAmount:sum(rows,'amount'),pending:rows.filter(row=>row.partner==='Partner no identificado'||row.reasons.length===0).length,employees,hours,topPartner:employees[0]||null,peakHour:hours[0]||null,filterOptions:{reasons:[...new Set(allVoids.map(row=>row.reason).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es')),partners:[...new Set(allVoids.map(optionPartner).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es')),weeks:[...new Set(allVoids.map(row=>weekKey(row.dateKey)))].sort(),weekdays:[...new Set(allVoids.map(row=>row.weekday))].sort()},...dateExtent(voids)};
}
...
export function reportFor(module,result,context={}){
 ...
 if(module==='peak'){
  const cycleMode=context.peakView==='Tarea de Ciclo',trendMode=context.peakView==='PH Tendencia';report.layout=cycleMode?'cycle-day-plan':trendMode?'peak-trend':'peak-hour-plan';report.hideSummary=true;report.operationalHeader=true;report.generatedAt=context.today||new Date().toISOString().slice(0,10);report.summary=[];
  if(cycleMode){report.title='Tarea de Ciclo';report.cycleDay=context.cycleDay||'';report.cycleDate=context.cycleDate||'';report.cycleRows=context.cycleRows||[];sheet('Tarea de Ciclo',['Time Period','TX','CS · Frecuencia de ciclo','Actividad sugerida'],report.cycleRows.map(x=>[x.label,x.tx,`${x.cycleMinutes} min`,x.activity]));}
  else if(trendMode){const trend=context.peakTrend||{columns:[],rows:[],day:''},dates=trend.columns.map(column=>`${column.day} ${shortDate(column.date)}`),point=peak=>peak?`${peak.label} · ${peak.total} TX`:'',lastDelta=value=>value==null?'':value;report.title='PH Tendencia';report.peakTrend=trend;report.filters=[`${trend.day||'Día'} comparable`,report.filters].filter(Boolean).join(' · ');sheet('PH Tendencia',['Métrica / Time Period',...dates,'Δ última semana'],[['Peak AM · 4 medias horas',...trend.columns.map(column=>point(column.am)),lastDelta(trend.latest?.am&&trend.previous?.am?trend.latest.am.total-trend.previous.am.total:null)],['Peak PM · 4 medias horas',...trend.columns.map(column=>point(column.pm)),lastDelta(trend.latest?.pm&&trend.previous?.pm?trend.latest.pm.total-trend.previous.pm.total:null)],['Dif. semana anterior · total día',...trend.columns.map(column=>lastDelta(column.delta)),lastDelta(trend.latest?.delta)],...trend.rows.map(row=>[row.label,...row.values,lastDelta(row.delta)])]);}
  else{report.title='Peak Hour';report.peakBasis=context.peakBasis||'average';report.peakRows=context.peakRows||[];report.timePeriodRows=r.activeSlots.map(x=>({slot:x.slot,label:`${x.label}-${clock(x.slot+1)}`,total:x.total,average:x.average,weekday:x.weekday}));sheet('Tabla comparativa',['Día','Peak AM','Prom. AM','Obj. AM (+5)','Real AM','Peak PM','Prom. PM','Obj. PM (+5)','Real PM'],report.peakRows.map(x=>[`${x.day} ${shortDate(x.date)}`,x.am?.label||'',x.am?.average??'',x.am?.target??'',x.realAm??'',x.pm?.label||'',x.pm?.average??'',x.pm?.target??'',x.realPm??'']));sheet('Time Period',['Time Period','TX total','Promedio',...DAY_LABELS],report.timePeriodRows.map(x=>[x.label,x.total,x.average,...x.weekday.map(value=>value>0?value:'')]));}
 }
 if(module==='normal'){
 ...
 if(module==='audit'){
  report.summary=[['Cheques con línea Void',r.voidCount],['Void exclusivo confirmado',r.exclusiveCount],['Riesgo de borrado',r.deletionCount],['Riesgo inferido por pagos',r.inferredDeletionCount],['Ticket reabierto',r.reopenCount],['Sin cierre verificado',r.unverifiedCount]];
  sheet('Clasificación',['Fecha','Hora','Ticket','Partner','Puesto','Clasificación','Reabierto','Motivo','Líneas negativas','Total ticket','Neto pagos','Evidencia','Importe de revisión'],r.items.map(x=>[x.date,new Date(x.ms).toISOString().slice(11,19),x.ticket,x.partner,x.position,x.risk,x.reopen?'Sí':'No',x.reasons.join(', '),x.negativeLines,x.ticketTotal,x.paymentTotal,x.evidence,x.amount]));
  sheet('Líneas del ticket',['Fecha','Hora','Ticket','Clasificación','Reabierto','SecTrans','SecDtl','IDProducto','Producto','Cantidad','Importe línea','Motivo','Partner'],r.items.flatMap(x=>x.lines.map(line=>[x.date,new Date(line.at).toISOString().slice(11,19),x.ticket,x.risk,x.reopen?'Sí':'No',line.secTrans,line.secDtl,line.product,line.name,line.quantity,line.total,line.reason,line.employee])));
