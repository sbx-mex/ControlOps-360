...
export const STRUCTURES = Object.freeze({
 ...
 auditLegacy: ["IDTienda","Ticket","FechaCierre","IDEmpleado","Total","Partner","Puesto_asignado","Forma de Pago","Reason","Producto"],
 product: ["IDProducto","Descripcion"], store: ["IDTienda","Tienda"],
 employee: ["IDTienda","IDEmpleado","Nombre","Puesto"],
...
export const FACT_FIELDS={sales:"salesFacts",usage:"usageFacts",auditTicket:"auditTickets",auditVoid:"auditVoids",auditPayment:"auditPayments",auditLegacy:"auditLegacyRows"};
export const FACT_TYPES=Object.keys(FACT_FIELDS);
export const CATALOG_FIELDS={product:"productCatalog",store:"storeCatalog",employee:"employeeCatalog",stock:"stockCatalog",sapList:"sapCatalog",microsList:"microsCatalog",compostable:"compostableCatalog",woe:"woeCatalog",food:"foodRules",drink:"drinkRules",cream:"creamRules",baking:"bakingCatalog",storePolicy:"storePolicies"};
...
function addFacts(d,type,headers,rows,source){
 ...
 for(const row of rows){d.sourceRows++;const get=getter(row),store=cleanId(get("IDTienda")),ticket=cleanId(get("Ticket")),date=dateParts(type==="usage"?get("Fecha"):type==="auditTicket"?get("FechaCierre","Fecha"):type==="auditLegacy"?get("FechaCierre"):get("FechaHora")),fact={store,...date};
...
export function addReferenceRows(d,type,headers,rows,source={}){
 ...
 for(const row of rows){const get=getter(row);let key,value;
  ...
  if(type==="employee"){const store=cleanId(get("IDTienda")),id=cleanId(get("IDEmpleado"));key=`${store}|${id}`;value={store,id,name:String(get("Nombre","Partner")).trim(),position:String(get("Puesto","Puesto_asignado","CodTrabDesc")).trim()};}
...
export function workbookKind(d){const types=d?.sourceTypes||new Set(),references=d?.referenceTypes||new Set(),groups=[];if(types.has("sales"))groups.push("sales");if(types.has("usage"))groups.push("usage");if(["auditTicket","auditVoid","auditPayment","auditLegacy"].some(type=>types.has(type)))groups.push("audit");if(!groups.length&&["sapList","microsList"].some(type=>references.has(type)))groups.push("priceList");return groups.sort().join("+");}
...
export function workbookPeriod(record){let min=Infinity,max=-Infinity;for(const name of Object.values(FACT_FIELDS)){for(const row of record?.dataset?.[name]||[]){if(Number.isFinite(row.dayMs)){min=Math.min(min,row.dayMs);max=Math.max(max,row.dayMs);}}}const from=Number.isFinite(min)?new Date(min).toISOString().slice(0,10):"",to=Number.isFinite(max)?new Date(max).toISOString().slice(0,10):"";return {from,to,key:from&&to?`${from}|${to}`:"sin-periodo"};}
export function workbookCeCo(record){const stores=[...operationalStores(record?.dataset||createDataset())];return stores.length===1?stores[0]:stores.length?"MIXTO":"";}
export function workbookIdentity(record){const kind=workbookKind(record?.dataset),references=[...(record?.dataset?.referenceTypes||[])].sort().join("+")||"sin-tipo",ceco=workbookCeCo(record)||"SIN-CECO",period=workbookPeriod(record).key;return kind?`${ceco}|${kind}|${period}`:`PARAMETRO|${references}|${period}`;}
...
export function selectRecentWorkbooks(records){const winners=new Map(),superseded=[],decisions=[];for(const record of records){const identity=workbookIdentity(record),current=winners.get(identity);if(!current){winners.set(identity,record);continue;}const winner=compareWorkbookFreshness(record,current)>0?record:current,ignored=winner===record?current:record;winners.set(identity,winner);superseded.push(ignored);decisions.push({identity,winner,ignored,reason:"Mismo CeCo, tipo y periodo; se conservó la versión más reciente."});}const selected=[...winners.values()].sort((a,b)=>{const kindA=workbookKind(a.dataset),kindB=workbookKind(b.dataset);if(!kindA!==!kindB)return kindA?1:-1;const periodA=workbookPeriod(a),periodB=workbookPeriod(b);return periodA.from.localeCompare(periodB.from)||periodA.to.localeCompare(periodB.to)||workbookIdentity(a).localeCompare(workbookIdentity(b))||compareWorkbookFreshness(a,b);});return {selected,superseded,decisions};}
