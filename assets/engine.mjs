...
export const CATALOG_FIELDS={product:"productCatalog",store:"storeCatalog",stock:"stockCatalog",compostable:"compostableCatalog",woe:"woeCatalog",food:"foodRules",drink:"drinkRules",cream:"creamRules",baking:"bakingCatalog",storePolicy:"storePolicies"};
...
export function addReferenceRows(d,type,headers,rows,source={}){
 ...
 for(const row of rows){const get=getter(row);let key,value;
  ...
  if(type==="food"){key=normalize(get("Item"));value={food:booleanValue(get("Alimento")),pieces:numberValue(get("#Alimento")),bis:booleanValue(get("BIS")),bakingName:String(get("Nombre Unificado BIS")).trim(),assembly:String(get("Nombre Unificado Ensamble")).trim()};if(value.food===null||value.bis===null||!(value.pieces>0)||(value.bis&&!value.bakingName))throw new Error("Regla de alimento sin clasificación, factor o nombre BIS válido.");}
...
export function mergeDataset(target,source){
 ...
 for(const type of source.referenceTypes){const name=CATALOG_FIELDS[type];target[name]=new Map(source[name]);target.referenceTypes.add(type);if(source.references.has(type))target.references.set(type,source.references.get(type));}
...
export function workbookKind(d){const types=d?.sourceTypes||new Set(),groups=[];if(types.has("sales"))groups.push("sales");if(types.has("usage"))groups.push("usage");if(["auditTicket","auditVoid","auditPayment"].some(type=>types.has(type)))groups.push("audit");return groups.sort().join("+");}
export function workbookFreshness(record){const days=new Set();let maxDay=-Infinity,rows=0;for(const name of Object.values(FACT_FIELDS)){for(const row of record?.dataset?.[name]||[]){rows++;if(row.dateKey)days.add(row.dateKey);if(Number.isFinite(row.dayMs)&&row.dayMs>maxDay)maxDay=row.dayMs;}}return {maxDay,maxDate:Number.isFinite(maxDay)?new Date(maxDay).toISOString().slice(0,10):"",days:days.size,rows,lastModified:Number(record?.lastModified)||0,sequence:Number(record?.sequence)||0};}
export function compareWorkbookFreshness(left,right){const a=workbookFreshness(left),b=workbookFreshness(right);for(const key of ["maxDay","days","lastModified","rows","sequence"]){if(a[key]!==b[key])return a[key]>b[key]?1:-1;}return String(left?.fingerprint||left?.name||"").localeCompare(String(right?.fingerprint||right?.name||""));}
