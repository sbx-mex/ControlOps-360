...
function createPeakPdf(report){
 const commands=[],number=value=>value==null?'-':NUMBER.format(value),delta=value=>value?`${value.value>0?'+':''}${NUMBER.format(value.value)}`:'-',tone=value=>value>0?'green':value<0?'red':'muted';
 const text=(x,y,size,value,bold=false,align='left',color='dark')=>{const source=String(value??''),at=align==='center'?x-source.length*size*.27:align==='right'?x-source.length*size*.54:x,ink=color==='white'?'1 1 1':color==='muted'?'0.34 0.43 0.38':color==='gold'?'0.66 0.49 0.19':color==='green'?'0 0.38 0.25':color==='red'?'0.56 0.25 0.17':'0.06 0.12 0.10';commands.push(`BT /F${bold?2:1} ${size} Tf ${ink} rg ${at} ${y} Td (${pdfEscape(source)}) Tj ET`);};
 const box=(x,y,w,h,fill='warm',stroke='0.84 0.78 0.66')=>{const color=fill==='green'?'0.02 0.31 0.22':fill==='soft'?'0.94 0.96 0.92':fill==='white'?'1 1 1':'0.98 0.95 0.88';commands.push(`${color} rg ${x} ${y} ${w} ${h} re f ${stroke} RG 0.5 w ${x} ${y} ${w} ${h} re S`);};
 const peak=report.peak||{},fallbackGoal=value=>{const current=Number.isFinite(value?.average)?Math.round(value.average):null;return {current,target:current==null?null:current+5,increase:5,progress:current==null?null:current/(current+5)};},goals={am:report.goals?.am||fallbackGoal(peak.am),pm:report.goals?.pm||fallbackGoal(peak.pm)};
 drawOperationalHeader(commands,text,report);text(28,548,17,'PEAK HOUR | REPORTE VISUAL',true);text(28,532,6.8,[report.period,report.filters,`${number(peak.orders)} órdenes | ${number(peak.days)} días`].filter(Boolean).join(' | '),false,'left','muted');box(646,536,118,20,'green','0.02 0.31 0.22');text(705,543,6.2,'UNA SOLA LÁMINA',true,'center','white');
 [['AM',peak.am,goals.am],['PM',peak.pm,goals.pm]].forEach(([label,value,goal],index)=>{const x=28+index*378,y=425,w=358,h=88;box(x,y,w,h,index?'soft':'warm');text(x+15,y+70,6.2,`OBJETIVO ${label} | ${value?.label||'Sin demanda'}`,true,'left',index?'green':'gold');text(x+15,y+47,6.2,'PROMEDIO ACTUAL',true,'left','muted');text(x+15,y+20,22,number(goal.current),true);text(x+116,y+29,15,'>',true,'center','gold');text(x+145,y+47,6.2,'PRÓXIMO OBJETIVO',true,'left','muted');text(x+145,y+20,22,number(goal.target),true,'left','green');box(x+w-80,y+49,64,22,'green','0.02 0.31 0.22');text(x+w-48,y+57,7.2,`+${number(goal.increase)}`,true,'center','white');const progress=Math.max(0,Math.min(1,goal.progress||0));commands.push(`0.87 0.86 0.80 rg ${x+255} ${y+22} 86 6 re f 0 0.38 0.25 rg ${x+255} ${y+22} ${86*progress} 6 re f`);text(x+298,y+10,5.5,'avance hacia meta',false,'center','muted');});
 text(28,402,9.6,'TENDENCIA | ÚLTIMO VS ANTERIOR COMPARABLE',true);text(764,402,6,'AM y PM suman cuatro medias horas consecutivas',false,'right','muted');
 const comparisons=(report.comparisons||[]).slice(0,7),gap=6,cardW=(736-gap*6)/7;
 if(!comparisons.length){box(28,326,736,59,'white');text(396,351,7.2,'Aún no hay dos fechas del mismo día para validar tendencia.',false,'center','muted');}
 comparisons.forEach((item,index)=>{const x=28+index*(cardW+gap),combined=(item.amDelta?.value||0)+(item.pmDelta?.value||0);box(x,326,cardW,59,combined<0?'warm':combined>0?'soft':'white');text(x+8,371,7.2,item.day.toUpperCase(),true,'left',tone(combined));text(x+8,360,5.3,`${shortPdfDate(item.current.date)} vs ${shortPdfDate(item.previous.date)}`,false,'left','muted');text(x+8,343,6.4,`AM ${delta(item.amDelta)}`,true,'left',tone(item.amDelta?.value||0));text(x+cardW-8,343,6.4,`PM ${delta(item.pmDelta)}`,true,'right',tone(item.pmDelta?.value||0));});
 text(28,304,9.6,'LAS CUATRO MEDIAS HORAS QUE MUEVEN EL PEAK',true);text(764,304,6,'Sin ceros y sin franjas ajenas al objetivo',false,'right','muted');
 const slotLabel=slot=>`${String(Math.floor(slot/2)).padStart(2,'0')}:${slot%2?'30':'00'}`;
 [['AM',peak.am],['PM',peak.pm]].forEach(([label,value],index)=>{const x=28+index*378,y=218,w=358,h=70;box(x,y,w,h,index?'soft':'warm');text(x+12,y+54,6.2,`PEAK ${label} | ${value?.label||'Sin demanda'}`,true,'left',index?'green':'gold');if(value){const cellW=80;value.halfHours.forEach((amount,slotIndex)=>{const cellX=x+12+slotIndex*(cellW+5);commands.push(`1 1 1 rg ${cellX} ${y+9} ${cellW} 34 re f`);text(cellX+cellW/2,y+29,5.4,slotLabel(value.slot+slotIndex),false,'center','muted');text(cellX+cellW/2,y+15,9.5,amount===0?'':number(amount),true,'center',amount===value.halfHourMax?'green':'dark');});}else text(x+w/2,y+25,8,'Sin demanda en este bloque.',false,'center','muted');});
 const signals=comparisons.flatMap(item=>['am','pm'].map(segment=>({day:item.day,segment:segment.toUpperCase(),date:item.current.date,delta:item[`${segment}Delta`]})).filter(item=>item.delta)),best=signals.filter(item=>item.delta.value>0).sort((a,b)=>b.delta.value-a.delta.value)[0],watch=signals.filter(item=>item.delta.value<0).sort((a,b)=>a.delta.value-b.delta.value)[0];
 const decision=(x,title,item,empty,color)=>{box(x,82,230,111,color==='green'?'soft':color==='red'?'warm':'white');text(x+14,173,6.2,title,true,'left',color);if(item){text(x+14,146,14,`${item.day} | ${item.segment}`,true);text(x+14,121,19,`${item.delta.value>0?'+':''}${number(item.delta.value)}`,true,'left',color);text(x+82,126,6.2,'órdenes vs anterior',false,'left','muted');text(x+14,99,5.8,`Validado al ${shortPdfDate(item.date)}`,false,'left','muted');}else{text(x+14,141,9,empty,true);text(x+14,113,6,'Se completa con dos días comparables.',false,'left','muted');}};
 decision(28,'MAYOR IMPULSO',best,'Sin crecimiento confirmado','green');decision(270,'OPORTUNIDAD PRIORITARIA',watch,'Sin caída confirmada','red');box(512,82,252,111,'green','0.02 0.31 0.22');text(526,173,6.2,'SIGUIENTE PASO',true,'left','gold');text(526,148,11,'VALIDA LA EJECUCIÓN',true,'left','white');text(526,130,7,'Abre el módulo Asistente PH',true,'left','white');text(526,113,6,'Consulta frecuencia, horarios y 3S',false,'left','white');text(526,94,5.8,'La vista gerente y el asistente quedan separados.',false,'left','white');
 text(28,58,6.2,'LECTURA RÁPIDA',true,'left','gold');text(94,58,6.2,`Actual + impulso = objetivo. Ejemplo: ${number(goals.am.current)} + ${number(goals.am.increase)} = ${number(goals.am.target)}.`,false,'left','muted');
